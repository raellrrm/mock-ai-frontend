        ...
        <div className="flex items-center gap-2 shrink-0 border-l border-slate-100 pl-3">
          {!isRecording && (
            <button
              ...
              disabled={!text.trim()}
              className={`cursor-pointer flex h-9 w-9 items-center justify-center rounded-full transition-colors ${text.trim() ? 'bg-indigo-100 text-indigo-600 hover:bg-indigo-200' : 'bg-slate-100 text-slate-400'
                }`}
            >
          ...
          <button
            ...
            className={`cursor-pointer flex h-10 w-10 items-center justify-center rounded-full text-white shadow-lg transition-all active:scale-95 ${isRecording
